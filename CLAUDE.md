...
- ESP32 exclusive features: MQTT TLS/SSL, FreeRTOS background tasks (buzzer, HTTP server, crypto fetch, weather fetch)
...
- `buzzer_utils.cpp` - Audio feedback patterns (chirp library); `playChirpByName()` is non-blocking on ESP32 (posts to task queue), blocking on ESP8266
- `timer.cpp` - Countdown timer/stopwatch logic
- `main.cpp` - Entry point, initialization sequence

**ESP32 FreeRTOS Tasks** (Core 0, all non-blocking):
- `buzzer_task.cpp` - Plays chirp patterns via `ledcWriteTone` + `vTaskDelay`; queue depth 1, 3KB stack
- `http_task.cpp` - Runs `serverHttp.handleClient()` so HTTP page loads never block the display loop; 10KB stack. `handleHttpServer()` is a no-op on ESP32.
- Crypto fetch task (in `crypto.cpp`) - HTTPS fetches via binary semaphore trigger, writes to shadow buffer; 16KB stack
- Weather fetch task (in `weather.cpp`) - HTTP fetch via binary semaphore trigger, writes to `WeatherShadow` struct; 12KB stack
...
- **Non-blocking operations** (ESP32 only): buzzer playback, HTTP serving, crypto fetch, and weather fetch all run as FreeRTOS tasks on Core 0, leaving the display loop on Core 1 uninterrupted. On ESP8266, buzzer and HTTP remain synchronous (buzzer blocks for chirp duration; HTTP is handled cooperatively via `handleClient()` in the main loop).
- **Recurrent alarm display**: Both platforms show a brief `MODE_RECURRENT_ALARM` indicator (default `"* * *"`, configurable via `/api/recurrent_alarm/config` `displayMessage` field). On ESP32 the clock continues updating during the chirp; on ESP8266 the display freezes for the chirp duration but at least shows the indicator.
